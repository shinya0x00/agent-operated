...
- Scope: PR #1 / Issue #2
- Doctrine ref:
  [`a5ad793c7c8bc52eae82645799b621356e3e6650`](https://github.com/shinya0x00/doctrine/blob/a5ad793c7c8bc52eae82645799b621356e3e6650/DOCTRINE.md)

## Context

ADR-0006はportable coreをrepository integrationから分離したが、actor、GTP recovery、durable record gate、
wiring executor、handoffを同じAO固有判定の下へ置いた。この配置では、GTP、Doctrine、Merge Steward等を追加するほど
AOが外部正本の意味と実行方法を吸収し、第二のcanonical ownerまたは汎用executorになる。
...
GTP recoveryはGTP Operation、publication screeningはDoctrine Publication Operationとして配置する。Operation Receiptは
...
present / attached / fired / validatedを区別する原則は維持するが、その機械checkはDoctrine Operationが所有する。
...
- GTP、Doctrine、Merge Stewardのinterface変更は該当AdapterまたはOperationへ局所化される。
