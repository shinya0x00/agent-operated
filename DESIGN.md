...
1. `check_plan` — GTP recoveryからtarget-native planを作った後、plan公開または最初のmutationの早い方の前
...
GTP task projectionとplanをcanonical digestへ束縛した`CheckedPlan`はinvocation-localに保持する。新しいinvocation、
task projection変更、materialなplan変更は新しい`CheckedPlan`を要求する。これはmutation authorityではない。

Projection Batchはoptional Candidate Head、uniqueなtarget ref、immutable bytes、SHA-256 digestを持つ。host-owned sourceが
作った同じbatch objectをprivate check、Publication Screening、publisherへ渡し、publish直前にdigestを再検査する。
...
- expected Human Accountを、mutation前のActor Observationと同じversioned Actor Profile sourceから取得する。
...
blocked transitionはcallback call countがzeroであること、Projection Batchは同一objectとdigestでpublisherへ到達することを
portable testで観測する。
