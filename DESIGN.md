...
2. operation phase
3. Public Delivery RouteとPrivate Control Routeの選択
...
このOperationは公開適合全体またはsecret absenceを証明しない。resultは独立した利用価値を持つため、必要な
Candidate Headへ束縛してOperation Receiptへ格納できる。
...
Internal Policy GateはOperationではない。AOは次のtransitionでhost固定providerをin-processで呼ぶ。

1. `check_plan` — planをdurable artifactとして公開する前
2. `check_candidate` — 最初の実candidateが存在した後、およびHuman handoff前
3. `check_projection` — repository、PR、Issue、Receipt等の公開予定artifactを確定する前
...
`blocked`は対象transitionだけを止め、repair後の新しいplan、candidate、projectionを再検査する。
