...
| Hooks | user-level、managed、plugin-bundled `SessionStart`／`PreToolUse`等を利用できる | 状態表示、観測、対応toolの早期denyに使用可能。Hook単独をdeny authorityにしない |
| `PreToolUse` output | 対応toolは`permissionDecision: "deny"`、legacy `decision: "block"`、exit code 2で事前拒否できる。`continue: false`は非対応で、一部tool pathはdefault hook pathを外れ得る | 早期deny／observationに使用するが、唯一のfail-closed enforcementへ昇格しない |
...
| Hook Adapter | `SessionStart`で状態を表示し、`PreToolUse`でfindingを観測・提示して対応toolを早期denyする | user-level、managed、またはplugin-bundled Hook。write capabilityの正本にはしない |
